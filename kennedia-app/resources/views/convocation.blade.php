...
<body class="bg-white text-gray-900" style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;">
    ...
    <main class="max-w-5xl mx-auto px-4 py-6">
        <section class="grid grid-cols-3 gap-4 mb-6">
            <article class="relative overflow-hidden rounded-2xl border border-[#1F6F4A]/30 bg-white p-5 shadow-sm w-full min-h-[165px]">
        ...
        <section class="bg-white border border-gray-200 rounded-xl p-4 shadow-sm mb-6">
            ...
            <form id="uploadForm" class="space-y-3" method="POST" action="javascript:void(0);" onsubmit="return false;" data-max-upload-mb="{{ $effectiveUploadLimitMb }}">
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <input id="file" name="file" type="file" accept="application/pdf" required class="rounded-lg border border-gray-300 px-3 py-2 outline-none focus:border-[#1F6F4A]" />
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <input id="session" name="session" type="text" placeholder="e.g. 2021/2022" class="rounded-lg border border-gray-300 px-3 py-2 outline-none focus:border-[#1F6F4A]" />
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <select id="api_key_tier" name="api_key_tier" class="rounded-lg border border-gray-300 px-3 py-2 outline-none focus:border-[#1F6F4A]">
                ...
                <div class="flex gap-3">
                    <div class="flex-1 flex flex-col gap-2">
                        ...
                        <input id="page_start" name="page_start" type="number" min="1" placeholder="e.g. 1" class="rounded-lg border border-gray-300 px-3 py-2 outline-none focus:border-[#1F6F4A]" />
                    </div>
                    <div class="flex-1 flex flex-col gap-2">
                        <label for="page_end" class="font-medium">End Page (optional)</label>
                        <input id="page_end" name="page_end" type="number" min="1" placeholder="e.g. 10" class="rounded-lg border border-gray-300 px-3 py-2 outline-none focus:border-[#1F6F4A]" />
                    </div>
                </div>
                ...
                <div id="uploadProgress" class="hidden">
                    <div class="w-full bg-gray-200 rounded-full h-2.5">
                        <div id="progressBar" class="bg-[#9BC53D] h-2.5 rounded-full transition-all duration-300" style="width: 0%"></div>
                    </div>
                    <p id="progressText" class="text-sm text-gray-600 mt-2 text-center">Uploading...</p>
                </div>
                
                <button id="uploadBtn" type="submit" class="w-full py-3 bg-[#1F6F4A] text-white font-semibold rounded-lg hover:bg-[#9BC53D] transition disabled:opacity-50 disabled:cursor-not-allowed">
                    Upload and Extract
                </button>
            </form>
    ...
    <footer class="text-center text-[#3A3A3A] py-6">
