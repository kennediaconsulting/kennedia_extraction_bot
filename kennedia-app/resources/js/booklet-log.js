...
function renderRows(rows){
  ...
  for (const d of (Array.isArray(rows) ? rows : [])) {
    const start = d.page_start ?? ''
    const end = d.page_end ?? ''
    const requested = d.pages_requested ?? 0
    const processed = d.pages_processed ?? 0
    const pages = `${start || '-'}-${end || '-'} (requested: ${requested}) / processed: ${processed}`
...
async function loadBookletLogs(){
  ...
  try {
    if (msg) { msg.textContent = 'Loading booklet logs...'; msg.className = 'mt-3 text-sm text-gray-600' }
